...
public class Graphe implements Cloneable{
	...
	private Deque<Sommet> spillers = new ArrayDeque<Sommet>();
	...
	public Object clone() throws CloneNotSupportedException{
		...
		clonedGraphe.spillers = new ArrayDeque<Sommet>();
		clonedGraphe.spillers.addAll(this.spillers);
	 ...
	 * du graphe
	 */
	public void supprimerSommet(Sommet sommet) throws SommetNotFoundException{
	 ...
	 * du graphe
	 */
	public void supprimerArete(Arete arete) throws AreteNotFoundException{
	 ...
	 * @throws SommetNotFoundException si le sommet n'est pas un sommet du graphe
	 * @throws NegativeOrNullColorException si le nombre de couleurs à tester est négatif ou nul 
	 */
	public boolean estTrivialColorable(Sommet sommet, int k) 
			throws SommetNotFoundException, 
			NegativeOrNullColorException{
		if (k <= 0)
			throw new NegativeOrNullColorException("Erreur : " + k 
	...
	public Sommet trivialColorable(int k)
			throws NegativeOrNullColorException,
	...
	/**
	 * attribut une couleur k à un sommet du graphe
		...
		if (this.voisins(sommet).size() == 0)
			...
			for (Sommet voisin : this.voisins(sommet)) {
				...
				while(couleur <= k && couleur == voisin.getCouleur())
					couleur++;
				if (couleur > k) {
					throw new NotColorableException("Erreur : "
							+ "le graphe n'est pas " + k + "- colorable");
				}
				else 
	...
	/**
	 * ajoute le sommet "sommet" à la liste des sommets "spillés"
	 * @param sommet le sommet qui a spillé
	 */
	private void ajouteSpiller(Sommet sommet){
		spillers.push(sommet);
	}
}
